...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create yala-superfly-ddbc
  fly apps create yala-superfly-ddbc-staging  # ** not mandatory if you don't want a staging environnement **
  ```
...
- Add a `SESSION_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE`,`SUPABASE_ANON_PUBLIC`, `SERVER_URL` and `DATABASE_URL` to your fly app secrets
  ...
  > **Note:** To find your `SERVER_URL`, go to [your fly.io dashboard](https://fly.io/apps/yala-superfly-ddbc-3a36)
  ...
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app yala-superfly-ddbc-staging
  fly secrets set SUPABASE_URL="https://{YOUR_STAGING_INSTANCE_NAME}.supabase.co" --app yala-superfly-ddbc-staging
  fly secrets set SUPABASE_SERVICE_ROLE="{STAGING_SUPABASE_SERVICE_ROLE}" --app yala-superfly-ddbc-staging
  fly secrets set SUPABASE_ANON_PUBLIC="{STAGING_SUPABASE_ANON_PUBLIC}" --app yala-superfly-ddbc-staging
  fly secrets set DATABASE_URL="postgres://postgres:{STAGING_POSTGRES_PASSWORD}@db.{STAGING_YOUR_INSTANCE_NAME}.supabase.co:5432/postgres" --app yala-superfly-ddbc-staging
  fly secrets set SERVER_URL="https://{YOUR_STAGING_SERVEUR_URL}" --app yala-superfly-ddbc-staging
